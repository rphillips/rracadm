...
static void racadm_transport_reset(racadm_transport_t * t)
{
  free(t->data);
  t->data = NULL;
  t->size = 0;
...
static int racadm_login(racadm_transport_t * t)
{
  char url[1024];
  char data[1024];
...
static int racadm_logout(racadm_transport_t * t)
{
  char url[1024];
  racadm_transport_reset(t);
...
static int racadm_cmd(racadm_transport_t * t, const char *cmd)
{
  char url[1024];
  char data[1024];
  ...
  snprintf(url, sizeof(url), "https://%s:%i/cgi-bin/exec", t->conf->host,
           t->conf->port);
  snprintf(data, sizeof(data),
...
static char *racadm_parse(racadm_transport_t * t, const xmlChar * xpathExpr)
{
  char *result = NULL;
  xmlDocPtr doc;
...
static void racadm_setup_cookie(racadm_transport_t * t, const char *sid)
{
  char cookie[1024];
